...
from codes.utils.array_helpers import convert_to_array, convert_to_arrays, flatten_and_remove_nans, move_and_rescale
import json
...
class MPF:
    """
    This class should be used to compute the mean potential fluctuations
    """
    _methods = [
        'without adaptation',
        'with adaptation',
    ...
    def exc_conductance_mean(self, nu_e):
        """Calculates the mean excitatory conductance in [nS]."""
        return nu_e * self.num_e * (self.tau_e * 1e-3) * self.weight_e

    def exc_conductance_std(self, nu_e):
        """Calculates the standard deviation of the excitatory conductance in [nS]."""
        return np.sqrt(nu_e * self.num_e * (self.tau_e * 1e-3) / 2) * self.weight_e

    def inh_conductance_mean(self, nu_i):
        """Calculates the mean inhibitory conductance in [nS]."""
        return nu_i * self.num_i * (self.tau_i * 1e-3) * self.weight_i

    def inh_conductance_std(self, nu_i):
        """Calculates the standard deviation of the inhibitory conductance in [nS]."""
        return np.sqrt(nu_i * self.num_i * (self.tau_i * 1e-3) / 2) * self.weight_i
    
    def adaptation_mean(self, nu_e, nu_i):
        """Calculates the mean adaptation of the neuron in [pA]."""
        raise NotImplementedError("This method is not yet implemented")

    def conductance_mean(self, nu_e, nu_i):
        """Calculates the mean total conductance in [nS]."""
        return self.exc_conductance_mean(nu_e) + self.inh_conductance_mean(nu_i) + self.g_L

    def tau_eff(self, nu_e, nu_i):
        """Calculates the effective time constant of the neuron in [ms]."""
        return self.cm / self.conductance_mean(nu_e, nu_i) * 1e3

    @property
    def method(self):
        if not hasattr(self, '_method'):
            raise AttributeError(f"Attribute 'method' is not set. Please set it first. Can be one of {self._methods}.")
        return self._method
    
    @method.setter
    def method(self, value):
        if value not in self._methods:
            raise ValueError(f"Method should be one of {self._methods}")
        self._method = value

    def voltage_mean(self, nu_e, nu_i, nu_out=None, w=None):
        """Calculates the mean voltage of the neuron in [mV]."""
        exc_voltage = self.exc_conductance_mean(nu_e) * self.e_rev_E
        inh_voltage = self.inh_conductance_mean(nu_i) * self.e_rev_I
        return (exc_voltage + inh_voltage + self.g_L * self.v_rest) / self.conductance_mean(nu_e, nu_i)

    def _voltage_mean_with_adaptation(self, nu_e, nu_i, w):
        """Calculates the mean voltage of the neuron with adaptation in [mV]."""
        exc_voltage = self.exc_conductance_mean(nu_e) * self.e_rev_E
        inh_voltage = self.inh_conductance_mean(nu_i) * self.e_rev_I
        return (exc_voltage + inh_voltage + self.g_L * (self.v_rest + w)) / self.conductance_mean(nu_e, nu_i)
    
    def _voltage_mean_without_adaptation(self, nu_e, nu_i):
        """Calculates the mean voltage of the neuron without adaptation in [mV]."""
        return self.voltage_mean(nu_e, nu_i)
    
    def _voltage_mean_with_nu_out(self, nu_e, nu_i, nu_out):
        """Calculates the mean voltage of the neuron with nu_out in [mV]."""
        mu_V = self.voltage_mean(nu_e, nu_i)
        sigma_V = self.sigma_V(nu_e, nu_i)
        tau_VN = self.tau_eff(nu_e, nu_i) / self.tau_m
        return mu_V + np.sqrt(2) * sigma_V * erfcinv(2 * tau_VN * nu_out * 1e-3)
