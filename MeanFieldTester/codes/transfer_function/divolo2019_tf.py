...
Changes that had to be done in the original code:
- print statements were adjusted into Python 3 syntax
- plotting parts were commented out
...
class DiVolo2019TF(BaseTransferFunction):
    """
    Adapter class for the Di Volo 2019 Transfer Function.
    ...
    def required_inputs(self) -> List[str]:
        ...
        # to compute the membrane voltage fluctuations (muV).
        return ["exc_rate", "inh_rate", "adaptation"]

    def _get_legacy_params_dict(self) -> dict:
        """
        Translates the modern MFT network_params into the dictionary 
        format expected by the legacy Di Volo code.
        """
    ...
    def fit(self, single_neuron_results: SingleNeuronResults, **kwargs) -> dict:
        ...
        P_array = fitting_Vthre_then_Fout(
            ...
            Fe_eff=Fe_eff, 
            fiSim=fiSim, 
            w=w_eff,              # <-- Passed as XX inside the legacy code
            params=params,
    ...
    def evaluate(self, **kwargs) -> np.ndarray:
        """
        Maps standard kwargs (ve, vi, w) to TF_my_templateup.
        """
        fe = kwargs["exc_rate"]
        fi = kwargs["inh_rate"]
...
# original code source: https://github.com/ModelDBRepository/263236/tree/master
################################################################################
