...
# TODO: add the possibility to deal with equation object (stimulus saving)
...
def init(parameter_simulation,parameter_model,parameter_connection_between_region,parameter_coupling,
    ...
    model_class = eval(f"models.{parameter_model['model']}")
    model = model_class(variables_of_interest=model_class.state_variables)
 
    if hasattr(model_class, "C_m_e"):
        model.g_L_e=np.array(parameter_model['g_L_e'])
        model.C_m_e=np.array(parameter_model['C_m_e'])
        model.g_L_i=np.array(parameter_model['g_L_i'])
        model.C_m_i=np.array(parameter_model['C_m_i'])
    else:
        assert parameter_model['g_L_e'] == parameter_model['g_L_i'], "g_L_e and g_L_i should be the same"
        assert parameter_model['C_m_e'] == parameter_model['C_m_i'], "C_m_e and C_m_i should be the same"
        model.g_L=np.array(parameter_model['g_L_e'])
        model.C_m=np.array(parameter_model['C_m_e'])
    ...
    if "U_e" in parameter_model:
        model.U_e = np.array(parameter_model['U_e'])
        model.tau_rec_e = np.array(parameter_model['tau_rec_e'])
        model.tau_fac_e = np.array(parameter_model['tau_fac_e'])
        model.U_i = np.array(parameter_model['U_i'])
        model.tau_rec_i = np.array(parameter_model['tau_rec_i'])
        model.tau_fac_i = np.array(parameter_model['tau_fac_i'])
    ...
    if "second_order" in parameter_model['model']:
    ...
    model.stim_target_ratio=np.array(parameter_model['stim_target_ratio'])
    ...
    if parameter_connection_between_region['default']:
        ...
        if weights.ndim == 0:
            ...
            tract_lengths = tract_lengths.reshape(1,1)
        # optional file
        ...
        connection = lab.connectivity.Connectivity(
            ...
            region_labels=np.arange(0, parameter_connection_between_region['number_of_regions'], 1).astype('U128'),#TODO need to replace by parameter
    ...
    if parameter_stimulation is None:
        ...
        stimulation = lab.patterns.StimuliRegion(temporal=parameter_stimulation['eqn_t'],
    ...
    if parameter_coupling['type'] == 'Linear':
        ...
        coupling = lab.coupling.Sigmoidal(a=np.array(parameter_coupling['parameter']['a']),                                       
...
#%%
    ...
    parameter_simulation['path_result'].mkdir(parents=True, exist_ok=True)
    ...
    for name,dic in [('parameter_simulation',parameter_simulation),
        ...
        f.write('"'+name+'" : ')
        json.dump(dic, f, cls=PathJSONEncoder)
        f.write(",\n")
    if parameter_stimulation is not None:
        f.write('"parameter_stimulation" : ')
        # json.dump(parameter_stimulation['eqn_t'].parameters, f, cls=PathJSONEncoder)
        json.dump({"stimulation":True}, f, cls=PathJSONEncoder)
        f.write(",\n")
...
def run_simulation(simulator, time, parameter_simulation,parameter_monitor):
    ...
    for result in simulator(simulation_length=time):
        ...
        if result[0][0] >= parameter_simulation['save_time']*(count+1): #check if the time for saving at some time step
            ...
            np.save(parameter_simulation['path_result'] / f'step_{count}.npy', np.array(save_result,dtype=object),allow_pickle=True)
            save_result =[]
            for i in range(nb_monitor):
                save_result.append([])
            count +=1 
...
def get_result(path,time_begin,time_end, prints=0):
    ...
    for count in range(count_begin,count_end):
        ...
        result = np.load(path / f'step_{count}.npy',allow_pickle=True)
...
def get_region(path,time_begin,time_end,region_nb):
    ...
    for count in range(count_begin,count_end):
        result = np.load(path / f'step_{count}.npy')
        for i in range(result.shape[0]):
            tmp = np.array(result[i])
            if len(tmp) != 0:
                tmp = tmp[np.where((time_begin <= tmp[:,0]) &  (tmp[:,0]<= time_end)),:]
...
def print_bistability(parameter_model,show=True):
    ...
    if parameter_model['matteo']:
        import codes.tvb_models.Zerlaut_matteo as model
    else:
        import codes.tvb_models.Zerlaut as model
    ## Model
...
def print_space_variable(path,time_begin,time_end,position_monitor,limit=True):
    ...
    for region in range(output[position_monitor][1].shape[2]):
        ...
        plt.savefig(path / f'region{region}.png')
        plt.close('all')
